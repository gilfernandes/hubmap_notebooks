...
def LSUVinit(model,data, needed_std = 1.0, std_tol = 0.1, max_attempts = 10, do_orthonorm = True,needed_mean = 0., cuda = False, verbose = True, tqdm_enabled = True):
    cuda = data.is_cuda
    gg['total_fc_conv_layers']=0
    gg['done_counter']= 0
    gg['hook_position'] = 0
    ...
    with torch.no_grad():
        ...
        if cuda:
            model = model.cuda()
        for layer_idx in tqdm(range(gg['total_fc_conv_layers']), total = gg['total_fc_conv_layers'], disable = not tqdm_enabled):
